...
class TranslationTranslatorTest extends TestCase
{
    ...
    public function testStringMethodThrowsExceptionForArrayItem()
    {
        ...
        $this->expectExceptionMessageIsOrContains('Translation value for key [foo::bar.baz] must be a string, array given.');
    ...
    public function testArrayMethodThrowsExceptionForStringItem()
    {
        ...
        $t->getLoader()->shouldReceive('load')->once()->with('en', 'bar', 'foo')->andReturn(['baz' => 'breeze']);
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessageIsOrContains('Translation value for key [foo::bar.baz] must be an array, string given.');
