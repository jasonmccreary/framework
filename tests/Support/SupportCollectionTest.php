...
class SupportCollectionTest extends TestCase
{
    ...
    public function testNthThrowsExceptionForInvalidStep($collection)
    {
        ...
        $this->expectExceptionMessageIsOrContains('Step value must be at least 1.');
    ...
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessageIsOrContains('Number of groups must be at least 1.');
    ...
    public function testItThrowsExceptionWhenTryingToAccessNoProxyProperty($collection)
    {
        ...
        $this->expectExceptionMessageIsOrContains('Property [foo] does not exist on this collection instance.');
        $data->foo;
    }
    ...
    public function testEnsureForScalar($collection)
    {
        ...
        $this->expectException(UnexpectedValueException::class);
        $this->expectExceptionMessageIsOrContains("Collection should only include [int] items, but 'string' found at position 3.");
        $data->ensure('int');
    }
    ...
    public function testEnsureForObjects($collection)
    {
        ...
        $this->expectException(UnexpectedValueException::class);
        $this->expectExceptionMessageIsOrContains(sprintf('Collection should only include [%s] items, but \'%s\' found at position %d.', class_basename(new stdClass()), gettype($collection), 3));
        $data->ensure(stdClass::class);
    }
    ...
    public function testEnsureForInheritance($collection)
    {
        ...
        $this->expectException(UnexpectedValueException::class);
        $this->expectExceptionMessageIsOrContains(sprintf("Collection should only include [%s] items, but '%s' found at position %d.", \Throwable::class, get_class($wrongType), 2));
        $data->ensure(\Throwable::class);
    }
    ...
    public function testEnsureForMultipleTypes($collection)
    {
        ...
        $data = $collection::make([new \Error, new \Error, $wrongType]);
        $this->expectException(UnexpectedValueException::class);
        $this->expectExceptionMessageIsOrContains(sprintf('Collection should only include [%s] items, but \'%s\' found at position %d.', implode(', ', [\Throwable::class, 'int']), get_class($wrongType), 2));
        $data->ensure([\Throwable::class, 'int']);
    }
