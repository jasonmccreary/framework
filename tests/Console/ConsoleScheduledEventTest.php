...
class ConsoleScheduledEventTest extends TestCase
{
    ...
    {
        $app = TestDouble::for(Application::class)->passthru();
        ...
        $eventA = new Event(TestDouble::for(EventMutex::class), 'php foo');
        $eventB = new Event(TestDouble::for(EventMutex::class), 'php foo');
        $this->assertEquals(
    ...
    public function testTimeBetweenChecks()
    {
        ...
        $event = new Event(TestDouble::for(EventMutex::class), 'php foo', 'UTC');
