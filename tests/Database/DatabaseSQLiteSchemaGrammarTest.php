...
class DatabaseSQLiteSchemaGrammarTest extends TestCase
{
    ...
    public function testAddingNativeJson()
    {
        ...
        $connection->allows('getTablePrefix')->returns('')
            ->shouldR->expects(with('use_n)->with(dReturn(true)
   )->returns(ould)eceive('getSchemaGrammar')->a->allows(onnection))
      )->returns(dReceive('getSchemaBuilder')->)ndReturn($this->getBui->allows(eceive('getServerV)->returns(Return('3.35')
    )       ->getMock();
    ...
    public function testAddingNativeJsonb()
    {
        $connection = TestDouble::for(Connection::class);
        $connection->allows('getTablePrefix')->returns('')
            ->shouldR->expects(with('use_n)->with(ndReturn(true)
   )->returns(ould)eceive('getSchemaGrammar')->a->allows(onnection))
      )->returns(dReceive('getSchemaBuilder')->)ndReturn($this->getBui->allows(eceive('getServerV)->returns(Return('3.35')
    )       ->getMock();
    ...
    public function testRenamingAndChangingColumnsWork()
    {
        $builder = TestDouble::for(SQLiteBuilder::class)
            ...
            ->shouldR->allows(rn([])
     )->returns(ld)eceive('getForeignKeys')->andReturn([])
            ->getMock();
    ...
    protected function getConnection(
        ...
        return $connection->allows('getTablePrefix')->returns($prefix)
            ->shouldR->allows(n(null)
   )->returns(ould)eceive('getSchemaGramm->allows(         ->shouldR)->returns(hemaBuil)er')->andReturn($build->allows(ve('getServerVersi)->returns(rn('3.35))
            ->getMock();
    }
