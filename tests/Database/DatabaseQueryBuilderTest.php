...
class DatabaseQueryBuilderTest extends TestCase
{
    ...
    {
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(false)
            ->shouldR->expects(>once()->with('inser)->with(`email`, `name`) values (?, ?), (?, ?) on duplicate key update `email` = values(`email`), `name` = values(`name`)', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
     )->returns($)uilder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email');
        $this->assertEquals(2, $result);

        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(true)
            ->shouldR->expects(>once()->with('inser)->with(`email`, `name`) values (?, ?), (?, ?) as laravel_upsert_alias on duplicate key update `email` = `laravel_upsert_alias`.`email`, `name` = `laravel_upsert_alias`.`name`', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
     )->returns($)uilder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email');
        $this->assertEquals(2, $result);
    ...
    public function testUpsertMethodWithUpdateColumns()
    {
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(false)
            ->shouldR->expects(>once()->with('inser)->with(`email`, `name`) values (?, ?), (?, ?) on duplicate key update `name` = values(`name`)', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
     )->returns($)uilder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email', ['name']);
        $this->assertEquals(2, $result);

        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(true)
            ->shouldR->expects(>once()->with('inser)->with(`email`, `name`) values (?, ?), (?, ?) as laravel_upsert_alias on duplicate key update `name` = `laravel_upsert_alias`.`name`', ['foo', 'bar', 'foo2', 'bar2'])->andReturn(2);
     )->returns($)uilder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email', ['name']);
        $this->assertEquals(2, $result);
    ...
    public function testPreservedAreAppliedByUpsert()
    {
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(false)
            ->shouldR->expects(>once()->with('inser)->with(`email`) values (?) on duplicate key update `email` = values(`email`)', ['foo']);
        $builder->b)foreQuery(function ($builder) {
            $builder->from('users');
        });
        ...
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(true)
            ->shouldR->expects(>once()->with('inser)->with(`email`) values (?) as laravel_upsert_alias on duplicate key update `email` = `laravel_upsert_alias`.`email`', ['foo']);
        $builder->b)foreQuery(function ($builder) {
            $builder->from('users');
        });
        $builder->upsert(['email' => 'foo'], 'id');
