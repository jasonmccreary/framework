...
class DatabaseQueryBuilderTest extends TestCase
{
    ...
    public function testBasicSelectUseWritePdo()
    {
        ...
        $builder->getConnection()->expects('select')->with('select * from `users`', [], false, []);
        $builder->useWritePdo()->select('*')->from('users')->get();

        $builder = $this->getMySqlBuilderWithProcessor();
        $builder->getConnection()->expects('select')->with('select * from `users`', [], true, []);
    ...
    public function testHavingAggregate()
    {
        ...
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->expects('getDatabaseName');
    ...
    public function testHavingFollowedBySelectGet()
    {
        ...
        $builder->getProcessor()->allows('processSelect')->resolves(function ($builder, $results) {
            return $results;
        });
        $builder->from('item');
    ...
    public function testJoinLateral()
    {
        ...
        $sub->getConnection()->expects('getDatabaseName');
    ...
    {
        $builder = $this->getBuilder();
        $builder->getConnection()->allows('select')->returns([['exists' => 1]]);
        $results = $builder->from('users')->doesntExistOr(function () {
            return 123;
        });
        $this->assertSame(123, $results);
        $builder = $this->getBuilder();
        $builder->getConnection()->allows('select')->returns([['exists' => 0]]);
        $results = $builder->from('users')->doesntExistOr(function () {
    ...
    {
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(false)->expects('affectingStatement')->with('insert into `users` (`email`, `name`) values (?, ?), (?, ?) on duplicate key update `email` = values(`email`), `name` = values(`name`)', ['foo', 'bar', 'foo2', 'bar2'])->returns(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email');
        $this->assertEquals(2, $result);

        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(true)->expects('affectingStatement')->with('insert into `users` (`email`, `name`) values (?, ?), (?, ?) as laravel_upsert_alias on duplicate key update `email` = `laravel_upsert_alias`.`email`, `name` = `laravel_upsert_alias`.`name`', ['foo', 'bar', 'foo2', 'bar2'])->returns(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email');
        $this->assertEquals(2, $result);
    ...
    {
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(false)->expects('affectingStatement')->with('insert into `users` (`email`, `name`) values (?, ?), (?, ?) on duplicate key update `name` = values(`name`)', ['foo', 'bar', 'foo2', 'bar2'])->returns(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email', ['name']);
        $this->assertEquals(2, $result);

        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(true)->expects('affectingStatement')->with('insert into `users` (`email`, `name`) values (?, ?), (?, ?) as laravel_upsert_alias on duplicate key update `name` = `laravel_upsert_alias`.`name`', ['foo', 'bar', 'foo2', 'bar2'])->returns(2);
        $result = $builder->from('users')->upsert([['email' => 'foo', 'name' => 'bar'], ['name' => 'bar2', 'email' => 'foo2']], 'email', ['name']);
        $this->assertEquals(2, $result);
    ...
    public function testTruncateMethod()
    {
        ...
        $builder = $this->getSQLiteBuilder();
        $builder->getConnection()->allows('getSchemaBuilder->parseSchemaAndTable')->returns([null, 'users']);
        $builder->from('users');
        $this->assertEquals([
    ...
    public function testTruncateMethodWithPrefixAndSchema()
    {
        ...
        $builder = $this->getSQLiteBuilder(prefix: 'prefix_');
        $builder->getConnection()->allows('getSchemaBuilder->parseSchemaAndTable')->returns(['my_schema', 'users']);
    ...
    {
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(false)->expects('affectingStatement')->with('insert into `users` (`email`) values (?) on duplicate key update `email` = values(`email`)', ['foo']);
        $builder->beforeQuery(function ($builder) {
            $builder->from('users');
        });
        ...
        $builder = $this->getMySqlBuilder();
        $builder->getConnection()->allows('getConfig')->with('use_upsert_alias')->returns(true)->expects('affectingStatement')->with('insert into `users` (`email`) values (?) as laravel_upsert_alias on duplicate key update `email` = `laravel_upsert_alias`.`email`', ['foo']);
        $builder->beforeQuery(function ($builder) {
            $builder->from('users');
        });
        $builder->upsert(['email' => 'foo'], 'id');
    ...
    public function testMySqlUpdateWithJsonPreparesBindingsCorrectly()
    {
        ...
        $connection->expects('update')->with('update `users` set `options` = json_set(`options`, \'$."enable"\', false), `updated_at` = ? where `id` = ?',
                ['2015-05-26 22:02:06', 0]);
        $builder = new Builder($connection, $grammar, $processor);
        $builder->from('users')->where('id', '=', 0)->update(['options->enable' => false, 'updated_at' => '2015-05-26 22:02:06']);

        $connection->expects('update')->with('update `users` set `options` = json_set(`options`, \'$."size"\', ?), `updated_at` = ? where `id` = ?',
    ...
    {
        $builder = $this->getPostgresBuilder();
        $builder->getConnection()->expects('update')->with('update "users" set "options" = jsonb_set("options"::jsonb, \'{"name","first_name"}\', ?)', ['"John"']);
        $builder->from('users')->update(['users.options->name->first_name' => 'John']);

        $builder = $this->getPostgresBuilder();
        $builder->getConnection()->expects('update')->with('update "users" set "options" = jsonb_set("options"::jsonb, \'{"language"}\', \'null\')', []);
...
SQL;
    ...
    public function testDynamicWhere()
    {
        ...
        $builder = TestDouble::for(Builder::class)->makePartial();
    ...
    public function testBuilderThrowsExpectedExceptionWithUndefinedMethod()
    {
        ...
        $builder->getProcessor()->allows('processSelect')->returns([]);
    ...
    public function testCursorPaginate()
    {
        ...
        $builder->allows('newQuery')->resolves(function () use ($builder) {
