...
class ContainerCallTest extends TestCase
{
    ...
    public function testCallWithoutRequiredParamsThrowsException()
    {
        ...
        $this->expectExceptionMessageIsOrContains('Unable to resolve dependency [Parameter #0 [ <required> $foo ]] in class Illuminate\Tests\Container\ContainerTestCallStub');
    ...
    {
        $this->expectException(BindingResolutionException::class);
        $this->expectExceptionMessageIsOrContains('Unable to resolve dependency [Parameter #0 [ <required> $foo ]] in class Illuminate\Tests\Container\ContainerCallTest');
