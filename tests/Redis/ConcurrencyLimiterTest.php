...
class ConcurrencyLimiterTest extends TestCase
{
    ...
    public function testAcquireUsesHashTagsOnPredisClusterConnection()
    {
        ...
        $connection->expects('eval')->with(Mockery::on(fn ($s) => str_contains($s, 'mget')),
            2,
            '{limiter}1', '{limiter}2',
            '{limiter}', Mockery::any(), Mockery::any())->returns('{limiter}1');

        $connection->expects('eval')->with(Mockery::on(fn ($s) => str_contains($s, 'del')),
            1,
            '{limiter}1', Mockery::any())->returns(1);
    ...
    public function testAcquireUsesPlainKeysOnPredisNonClusterConnection()
    {
        ...
        $connection->expects('eval')->with(Mockery::on(fn ($s) => str_contains($s, 'mget')),
            2,
            'lock1', 'lock2',
            'lock', Mockery::any(), Mockery::any())->returns('lock1');

        $connection->expects('eval')->with(Mockery::on(fn ($s) => str_contains($s, 'del')),
            1,
            'lock1', Mockery::any())->returns(1);
