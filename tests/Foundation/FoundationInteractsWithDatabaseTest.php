...
class FoundationInteractsWithDatabaseTest extends TestCase
{
    ...
    public function testSeeInDatabaseDoesNotFindResults()
    {
        ...
        $this->expectExceptionMessageIsOrContains('The table is empty.');
    ...
    public function testSeeInDatabaseFindsNotMatchingResults()
    {
        ...
        $this->expectExceptionMessageIsOrContains('Found similar results: '.json_encode([['title' => 'Forge']], JSON_PRETTY_PRINT));
    ...
    public function testSeeInDatabaseFindsManyNotMatchingResults()
    {
        ...
        $this->expectExceptionMessageIsOrContains('Found similar results: '.json_encode(['data', 'data', 'data'], JSON_PRETTY_PRINT).' and 2 others.');
    ...
    {
        $this->expectException(ExpectationFailedException::class);
        $this->expectExceptionMessageIsOrContains('Failed asserting that table [products] matches expected entries count of 3. Entries found: 1.');
        $this->mockCountBuilder(true);
    ...
    {
        $this->expectException(ExpectationFailedException::class);
        $this->expectExceptionMessageIsOrContains('Failed asserting that any existing row');
